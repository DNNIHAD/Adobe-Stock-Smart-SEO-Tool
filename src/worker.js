// Cloudflare Worker - Single-user Login Gate + Static Assets
// Auth via PBKDF2(SHA-256) + HttpOnly session cookie.
// All configurable values are taken from wrangler.toml [vars].
...
async function makeSession(username, ttlSec, secret) {
...
<title>Login</title>
<style>
body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;background:#0b0f17;color:#fff;display:grid;place-items:center;height:100vh}
...
.input{width:100%;padding:10px 12px;border-radius:8px;border:1px solid #334066;background:rgba(0,0,0,0.2);color:#fff}
...
function clearCookie(url) {
  return `sid=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Strict; Domain=${url.hostname}`;
}
function setCookie(url, sid, ttl) {
  return `sid=${sid}; Path=/; Max-Age=${ttl}; HttpOnly; Secure; SameSite=Strict; Domain=${url.hostname}`;
}

export default {
  async fetch(request, env, ctx) {
    // Load config from env (wrangler.toml [vars])
    const USERNAME = env.USERNAME || "admin";
    const PASS_SALT_B64 = env.PASS_SALT_B64;
    const PASS_DERIVED_B64 = env.PASS_DERIVED_B64;
    ...
    if (url.pathname === "/auth/login") {
      ...
      if (request.method === "POST") {
        ...
        if (ct.includes("application/json")) {
          ...
          username = form.get("username") || ""; password = form.get("password") || ""; next = form.get("next") || "/";
        }
        ...
        if (!timingSafeEqual(derived, PASS_DERIVED_B64)) {
          return loginPage(url, "Invalid username or password.");
        }

        const sid = await makeSession(username, SESSION_TTL_SECONDS, SESSION_SECRET);
        const res = Response.redirect(new URL(next, url), 302);
        res.headers.set("Set-Cookie", setCookie(url, sid, SESSION_TTL_SECONDS));
        return res;
      }
    ...
    if (url.pathname === "/auth/logout") {
      ...
      res.headers.set("Set-Cookie", clearCookie(url));
    ...
    // Protect everything else (static assets served from /public)
    const ok = await checkSession(request.headers.get("Cookie"), SESSION_SECRET);
    if (!ok) return loginPage(url);

    // Serve static assets (index.html, js, css) once authenticated
